...
info "Using administrator email: $ADMIN_EMAIL"
...
MYSQL_ROOT_PASSWORD="DSzGPMhaEFh5Cy5tMoDiMw=="
MYSQL_DATABASE_PASSWORD="DSzGPMhaEFh5Cy5tMoDiMw=="
echo "Using MySQL root password: $MYSQL_ROOT_PASSWORD"
echo "Using MySQL database password: $MYSQL_DATABASE_PASSWORD"
# Prompt for MySQL root password
#read -s -p "Enter MySQL root password: " MYSQL_ROOT_PASSWORD
#echo

# Prompt for MySQL database password
#read -s -p "Enter MySQL database password: " MYSQL_DATABASE_PASSWORD
#echo
...
#  sudo debconf-set-selections <<< "mysql-server mysql-server/mysql_password password $MYSQL_ROOT_PASSWORD"
#  sudo debconf-set-selections <<< "mysql-server mysql-server/mysql_password_again password $MYSQL_ROOT_PASSWORD"
...
#  sudo mysql -u root -e "ALTER USER 'root'@'%' IDENTIFIED BY '$MYSQL_ROOT_PASSWORD';"

  # Create database and user
#  log "Creating MySQL database and user..."
#  sudo mysql -u root -p"$MYSQL_ROOT_PASSWORD" -e "CREATE DATABASE IF NOT EXISTS collective_souls CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
#  sudo mysql -u root -p"$MYSQL_ROOT_PASSWORD" -e "CREATE USER IF NOT EXISTS 'collective_souls'@'localhost' IDENTIFIED BY '$MYSQL_DATABASE_PASSWORD';"
#  sudo mysql -u root -p"$MYSQL_ROOT_PASSWORD" -e "GRANT ALL PRIVILEGES ON collective_souls.* TO 'collective_souls'@'localhost';"
...
DB_NAME=${DB_NAME:-collective_souls}
DB_USER=${DB_USER:-collective_souls}
DB_PASSWORD=${DB_PASSWORD:-$MYSQL_DATABASE_PASSWORD}
DB_STORAGE=${DB_STORAGE:-/var/www/collective-souls/database/collective_souls.sqlite}
...
if [ "$DB_TYPE" = "mysql" ]; then
  ...
  npm install --omit=dev --no-optional sqlite3
fi
...
if [ -d "$BACKEND_NODE_MODULES" ]; then
    ...
    if [ -f "$BACKEND_NODE_MODULES/sqlite3/build/Release/node_sqlite3.node" ]; then
        ...
        if file "$BACKEND_NODE_MODULES/sqlite3/build/Release/node_sqlite3.node" 2>/dev/null | grep -q "Windows"; then
            ...
            sudo rm -rf "$BACKEND_NODE_MODULES"
        fi
    fi
fi
