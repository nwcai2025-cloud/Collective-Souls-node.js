...
if [ -d "$PROJECT_ROOT" ]; then
    ...
    error "Application directory does not exist: $PROJECT_ROOT"
fi
...
# Check for frontend build - look in multiple locations
FRONTEND_BUILD_FOUND=false
if [ -d "$PROJECT_ROOT/frontend/dist" ]; then
    FRONTEND_DIR="$PROJECT_ROOT/frontend"
    FRONTEND_BUILD_FOUND=true
elif [ -d "$PROJECT_ROOT/dist" ]; then
    FRONTEND_DIR="$PROJECT_ROOT"
    FRONTEND_BUILD_FOUND=true
fi

if [ "$FRONTEND_BUILD_FOUND" = true ]; then
    log "✓ Frontend build exists at $FRONTEND_DIR/dist"
else
    # Try to build or copy the frontend
    ...
    if [ -d "$DEV_FRONTEND/dist" ]; then
        ...
        cp -r "$DEV_FRONTEND/dist" "$PROJECT_ROOT/frontend/" 2>/dev/null || true
        if [ -d "$PROJECT_ROOT/frontend/dist" ]; then
    ...
    if [ "$FRONTEND_BUILD_FOUND" != true ] && [ -d "$PROJECT_ROOT/frontend" ]; then
...
MYSQL_USER="bob"
MYSQL_PASSWORD="Legend@2021"
MYSQL_HOST="localhost"
MYSQL_PORT="3306"
MYSQL_DB="collective_souls"

if mysql -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" -h "$MYSQL_HOST" -P "$MYSQL_PORT" -e "SELECT 1" "$MYSQL_DB" > /dev/null 2>&1; then
